...
## 2026-05-31 - [In-Memory State Leakage Across UI Sessions]
**Vulnerability:** The application stored sensitive data (like the selected isotype, allotype, custom mutations, and generated FASTA sequences) in the root application state. When users navigated backwards or abandoned a session (e.g., via "Back" or "Main Menu"), the UI screens were popped but the application-level state was not explicitly cleared. This allowed stale, proprietary data from previous sessions to persist in memory and potentially leak or contaminate new sessions.
**Learning:** Navigating away from a UI component (like popping a screen) does not automatically garbage collect or reset application-level state variables that were modified during that session. In wizard-like interfaces where forward navigation retains state, backward navigation must explicitly wipe it.
**Prevention:** Always implement explicit state clearing logic within the UI navigation actions (like `action_back` or `action_quit_to_main`) to wipe sensitive in-memory application state variables when the user deliberately abandons or reverses their workflow.
